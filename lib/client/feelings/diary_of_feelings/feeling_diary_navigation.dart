...
import 'feeling_diary.dart';
import 'package:flutter/material.dart';


class FeelingDiaryNavigation extends StatefulWidget {
  final String feeling;

  const FeelingDiaryNavigation({Key? key, required this.feeling}): super(key: key);

  @override
  _FeelingDiaryNavigationState createState() => _FeelingDiaryNavigationState();
}

class _FeelingDiaryNavigationState extends State<FeelingDiaryNavigation> {
  // 변수명 뒤에 ?이 붙은것은 null이 될 수 있다는 의미
  Widget? screenView;
  DrawerIndex? drawerIndex;
  late String nowfeeling;

  // 이 위젯이 로드될때 필요한 초기화를 진행함.
  @override
  void initState() {
    nowfeeling = widget.feeling;

    drawerIndex = DrawerIndex.Testing;
    screenView = ViewFeelingDiary(feeling: nowfeeling);
    super.initState();
  }
